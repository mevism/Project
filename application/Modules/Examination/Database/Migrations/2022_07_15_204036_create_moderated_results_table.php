...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('moderated_results', function (Blueprint $table) {
            ...
            $table->foreign('class_code')->references('name')->on('classes')->onDelete('no action')->onUpdate('no action');
            $table->string('student_number', 16);
            $table->foreign('student_number')->references('student_number')->on('student_courses')->onDelete('no action')->onUpdate('no action');
