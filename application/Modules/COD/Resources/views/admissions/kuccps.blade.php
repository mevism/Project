...
{{--                                @dd($app->kuccpsApplicant)--}}
                                <tr>
                                    <td> {{ $app->kuccpsApplicant->sname }} {{ $app->kuccpsApplicant->fname }} {{ $app->kuccpsApplicant->mname }} </td>
{{--                                    <td> {{ $app->courses->getCourseDept->name }}</td>--}}
                                    <td> {{ $app->kuccpsApplicant->gender }}</td>
                                    <td> {{ $app->course_code }}</td>
                                    <td> {{ $app->course_name }}</td>
                                    <td>
                                        @if($app->approveKuccps === NULL)
                                            <span class="badge bg-primary"> <i class="fa fa-spinner"></i> pending</span>
                                        @elseif($app->approveKuccps->cod_status === 1)
                                    ...
                                    <td nowrap="">
                                        @if($app->approveKuccps === NULL)
                                            <a class="btn btn-sm btn-alt-success" onclick="return confirm('Are you sure you want to accept this student?')" data-toggle="click-ripple" href="{{ route('cod.acceptAdmJab', $app->id) }}"> Accept </a>
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="block block-rounded block-transparent mb-0">
                                                            ...
                                                            <div class="block-content fs-sm">
                                                                <form action="{{ route('cod.rejectAdmJab', $app->id) }}" method="post">
                                                                    @csrf
                                        ...
                                        @elseif($app->approveKuccps->cod_status === 1)
                                            <a class="btn btn-sm btn-alt-success" data-toggle="click-ripple" onclick="return confirm('Are you sure you want to submit this record?')" href="{{ route('cod.submitAdmJab', $app->id) }}"> Submit </a> |
                                            <a class="btn btn-sm btn-alt-danger m-2" href="#" data-bs-toggle="modal" data-bs-target="#modal-block-popin"> Reject</a>
                                            <div class="modal fade" id="modal-block-popin" tabindex="-1" role="dialog" aria-labelledby="modal-block-popin" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-popin" role="document">
                                                    <div class="modal-content">
                                        ...
                                        @elseif($app->approveKuccps->cod_status === 2)
                                            <a class="btn btn-sm btn-alt-success" data-toggle="click-ripple" onclick="confirm('Are you sure you want to accept this student?')" href="{{ route('cod.acceptAdmJab', $app->id) }}"> Accept </a>                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
